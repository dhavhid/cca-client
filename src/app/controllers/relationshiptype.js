 ...
 */
'use strict';
angular.module('cca-client').controller('RelationshipTypeCtrl', function($scope, $state, Api, Messages, localStorageService, CatalogsAll){
    $scope.data = [];
    $scope.pagination = {};
    ...
    $scope.get = function(page) {
        ...
        Api.get('catalogs/relationshiptype', _params).then(function(response){
    ...
    $scope.addNew = function() {
        $state.go('catalogs.relationshiptypenew');
    }
    $scope.toTrash = function() {
        ...
        if (Array.isArray(sy_ids) && sy_ids.length > 0) {
            swal(confirmObj, function (isConfirm) {
                if (isConfirm) {
                    if (sy_ids.length > 0) {
                        Api.delete('catalogs/relationshiptype/', {id: sy_ids.join(','), per_page: 5000}).then(function (response) {
                            $scope.data = response.data.data;
                            $scope.pagination = response.data.meta.pagination;
                            localStorageService.set('relationshiptype', $scope.data);
...
angular.module('cca-client').controller('RelationshipTypeNewCtrl', function($scope, $state, Api, Flash){
    $scope.RelationshipType = {};
    $scope.activeTitle = 'Nuevo';
    $scope.save = function() {
        $scope.$broadcast('show-errors-check-validity');
        if ($scope.frmRelationshipType.$invalid)
            return;
        ...
        Api.post('catalogs',{Catalog: {
            ...
            $state.go('catalogs.relationshiptype');
        },function(error){});
    }
});
angular.module('cca-client').controller('RelationshipTypeUpdateCtrl', function($scope, $state, $stateParams, Api, Flash){
    $scope.RelationshipType = {};
    $scope.activeTitle = 'Actualizar';
    $scope.get = function() {
        var id = $stateParams.id;
        if (!isNaN(id)) {
            Api.get('catalogs/relationshiptype/' + id).then(function(response){
